...
class Agent:
    def __init__(self, modelType, gameFilename, hyperparameter_sets, hyperparameter_index=3, IW_k=2, extra_atom_allowed=True):
        ...
        self.hyperparameters = hyperparameter_sets
        print('meters', self.hyperparameters)
    ...
    def playCurriculum(self, heatmap=False, level_game_pairs=None, make_movie=False):
        ...
        if not level_game_pairs:
            level_game_pairs_module = importlib.import_module(self.gameFilename)
            level_game_pairs = level_game_pairs_module.level_game_pairs
        episodes = []
...
if __name__ == "__main__":
    ...
    def read_game_file(filepath):
        with open(filepath, 'r') as file:
            content = file.read()
        return content
    filename = "examples/gridphysics/frogs.txt"
    game_description = read_game_file(filename)
    gameName = game_description
